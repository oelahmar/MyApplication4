...
public class Person implements Serializable, Comparable<Person>{
    ...
    public String toString(){
        return  "id :"+getId()+"- "+
                ...
                "adresse : "+getAdresse()+", "+
                "ville :"+getVille()+". "+
                "vaccin1 : "+(isVaccine1()? "le "+
                new SimpleDateFormat("dd/MM/yyyy").format(getDateVacc1()) : "non")+". "+
                "vaccin2 : "+(isVaccine2()?"le "+
                new SimpleDateFormat("dd/MM/yyyy").format(getDateVacc2()) : "non");
    }
    ...
    public int compareTo(Person p) {
        int a= this.id-p.getId();int b=this.nom.compareTo(p.getNom());
        return a!=0?a:b;
    }}
